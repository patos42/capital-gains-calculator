...
class FirstInFirstOutInventory:
    def _record_matches(self,
        ...
        if trade.quantity > _ROUNDING_TOLERANCE:  # If buy trade, check if short
            ...
            if current_balance[trade.asset_code] < -_ROUNDING_TOLERANCE:
                for past_trade in past_trades.values():
                    if past_trade.quantity < -_ROUNDING_TOLERANCE: # Skip other buy orders.
                        closed_amount_past_trade = min(trade_quantity, -past_trade.quantity)
                        closed_amount_current_balance = min(trade_quantity, -current_balance[trade.asset_code])
                        ...
                        trade_quantity -= closed_amount
                        matched_inventory.append(MatchedInventory(past_trade.date,
                                                                  past_trade.price,
                                                                  trade.date,
                                                                  trade.price,
                                                                  closed_amount))
                    ...
                    if abs(current_balance[trade.asset_code]) < _ROUNDING_TOLERANCE < abs(trade_quantity):
                        current_balance[trade.asset_code] += trade_quantity
                        trade.quantity = trade_quantity
                        trade_quantity = 0

                    if abs(trade_quantity) > _ROUNDING_TOLERANCE:
                        break
            ...
            if current_balance[trade.asset_code] > _ROUNDING_TOLERANCE:
                for past_trade in past_trades.values():
                    if past_trade.quantity > _ROUNDING_TOLERANCE:  # Skip other sell orders.
                        ...
                        past_trade.quantity -= closed_amount
                        trade_quantity += closed_amount
                        matched_inventory.append(MatchedInventory(past_trade.asset_code,past_trade.date,
                                                                  past_trade.price,
                                                                  trade.date,
                                                                  trade.price,
                                                                  closed_amount))
        ...
        if trade.quantity > _ROUNDING_TOLERANCE: # If trade not fully matched, then remainder gets added to inventory.
            past_trades[trade.date] = trade

        return matched_inventory

    def match_trades(self, trades: List[Trade]) -> List[MatchedInventory]:
        sortedTrades = sorted(trades, key=lambda t: t.date)
        matched_inventory : List[MatchedInventory] = []
