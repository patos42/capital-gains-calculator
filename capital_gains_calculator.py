...
class DiscountCapitalGainsTaxMethod(CapitalGainsTaxMethod):
    def calculate_taxable_gain(self, matched_inventory : MatchedInventory):
        ...
        if (matched_inventory.sell_date >= test_date):
            taxable_gain = (matched_inventory.buy_price/matched_inventory.sell_price - 1) * matched_inventory.quantity / 2 #HACK: AUD.USD ONLY!
        else:
            taxable_gain = (matched_inventory.buy_price/matched_inventory.sell_price - 1) * matched_inventory.quantity
